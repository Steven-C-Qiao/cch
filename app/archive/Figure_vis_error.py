...
output_path = 'Figures/vis/00134_Take1_exp_104/'
...
def remove_outliers(points, k=20, threshold=2.0):
    ...
    Returns:
        Filtered points array with outliers removed
    """
    if len(points) < k + 1:
        return points  # Not enough points to compute neighbors
    ...
    num_removed = len(points) - np.sum(mask)
    if num_removed > 0:
        print(f'  Removed {num_removed} outliers ({num_removed/len(points)*100:.1f}%)')
...
if __name__ == '__main__':
    ...
    # Find all gt_vp_*.ply files in the output_path directory
    ...
    for gt_file in gt_files:
        ...
        gt_ply_path = os.path.join(output_path, gt_file)
        pred_ply_path = os.path.join(output_path, pred_file)
        
        if not os.path.exists(gt_ply_path) or not os.path.exists(pred_ply_path):
            print(f'Warning: Missing files for timestep {timestep}, skipping...')
            continue
        
        # Load vertices from PLY files
        try:
            gt_vertices_np = load_ply_vertices(gt_ply_path)
            pred_vertices_np = load_ply_vertices(pred_ply_path)
        except Exception as e:
            print(f'Error loading PLY files for timestep {timestep}: {e}')
            continue
        
        # Remove statistical outliers from predicted point cloud
        print(f'Timestep {timestep}: Removing outliers from pred point cloud...')
        pred_vertices_np_original = pred_vertices_np.copy()
        pred_vertices_np = remove_outliers(pred_vertices_np, k=20, threshold=5.0)
        
        if len(pred_vertices_np) < 100:
            print(f'Warning: Too few points remaining after outlier removal ({len(pred_vertices_np)}), using original')
            pred_vertices_np = pred_vertices_np_original
        
        # Convert vertices to torch tensors and move to device
        gt_vertices = torch.tensor(gt_vertices_np, dtype=torch.float32).unsqueeze(0).to(device)  # (1, N, 3)
        pred_vertices = torch.tensor(pred_vertices_np, dtype=torch.float32).unsqueeze(0).to(device)  # (1, N, 3)
        ...
        output_img_path = os.path.join(output_path, f'error_visualization_{timestep}.png')
        ...
        print(f'Timestep {timestep}: {distance_pred2gt.mean():.4f} cm, {distance_gt2pred.mean():.4f} cm - Saved visualization to {output_img_path}')
