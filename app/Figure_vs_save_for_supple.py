...
def get_multiple_view_params(x, num_views=20, base_elev=10, base_azim=0):
    """Get multiple view parameters around the body.
    
    Args:
        x: Vertex positions (N, 3)
        num_views: Number of views to generate (default: 20)
        base_elev: Base elevation angle in degrees (default: 10)
        base_azim: Base azimuth angle in degrees (default: 0)
    
    Returns:
        List of (center, camera_pos, max_range) tuples for each view
    """
    max_range = np.array([
        x[:, 0].max() - x[:, 0].min(),
        x[:, 1].max() - x[:, 1].min(),
    ...
    distance = max_range * 4.0
    elev_rad = np.radians(base_elev)
    
    view_params = []
    for i in range(num_views):
        # Distribute views evenly around 360 degrees
        azim = base_azim + (360.0 * i / num_views)
        azim_rad = np.radians(azim)
        ...
        view_params.append((center, camera_pos, max_range))
...
def render_and_save_multiple_views(verts, faces, colors=None, output_dir='output_views', 
                                    num_views=20, base_elev=10, base_azim=0,
                                    opacity=1.0, default_color='lightgray', 
                                    canvas_size=(1200, 800), prefix='view'):
    """Render and save multiple views around the body.
    
    Args:
        verts: Vertex positions (N, 3)
        faces: Face indices (M, 3) or None
        colors: Vertex colors (N, 3) or None
        output_dir: Directory to save images
        num_views: Number of views to render (default: 20)
        base_elev: Base elevation angle in degrees (default: 10)
        base_azim: Base azimuth angle in degrees (default: 0)
        opacity: Mesh opacity (default: 1.0)
        default_color: Default color if no colors provided (default: 'lightgray')
        canvas_size: Canvas size for rendering (default: (1200, 800))
        prefix: Prefix for output filenames (default: 'view')
    ...
    # Get view parameters for all views
    view_params_list = get_multiple_view_params(verts, num_views=num_views, 
                                                 base_elev=base_elev, base_azim=base_azim)
    
    saved_paths = []
    
    print(f'Rendering {num_views} views around the body...')
    for i, (center, camera_pos, max_range) in enumerate(view_params_list):
        # Render mesh from this viewpoint
        ...
        if (i + 1) % 5 == 0:
            print(f'  Saved {i + 1}/{num_views} views...')
    
    print(f'All {num_views} views saved to {output_dir}')
    return saved_paths
...
def main(base_dir, stride=1, rows_to_plot=['GT', 'UP2You', 'Ours'], output_path='Figures/vs_comparison.png', save_dpi=300, canvas_size=(1200, 800), render_20_views=False, views_output_dir=None):
    ...
    Args:
        ...
        render_20_views: If True, render and save 20 views around the body for each mesh
        views_output_dir: Base directory for saving 20-view images (default: base_dir/views_20)
    """
    ...
    # Render and save 20 full comparison figures with rotated views if requested
    if render_20_views:
        if views_output_dir is None:
            views_output_dir = os.path.join(base_dir, 'views_20')
        os.makedirs(views_output_dir, exist_ok=True)
        
        print('\n' + '='*60)
        print('Rendering 20 full comparison figures with rotated views...')
        print('='*60)
        ...
        for view_idx in range(num_views):
            ...
            # Re-render all images with this rotation angle
            gt_normals_images_rotated = []
            up2you_normals_images_rotated = []
            ours_normals_images_rotated = []
            gt_error_images_rotated = []
            up2you_error_images_rotated = []
            ours_error_images_rotated = []
            gt_rotated_images_rotated = []
            up2you_rotated_images_rotated = []
            ours_rotated_images_rotated = []
            
            # Render normals images with rotation
            if 'GT' in rows_to_plot:
                for idx, geom in enumerate(gt_normals_geoms):
                    if geom is not None:
                        verts, faces, colors, label = geom
                        center, camera_pos, max_range = get_view_params_rotated(verts, rotation_degrees=rotation_degrees)
                        img = render_mesh_pyvista(verts, faces, colors=colors, center=center, camera_pos=camera_pos,
                                                 max_range=max_range, opacity=1.0, default_color='lightgray', canvas_size=canvas_size)
                        if img.shape[2] == 3:
                            img = np.concatenate([img, np.full((*img.shape[:2], 1), 255, dtype=np.uint8)], axis=2)
                        gt_normals_images_rotated.append(img)
                    else:
                        gt_normals_images_rotated.append(None)
            ...
            if 'UP2You' in rows_to_plot:
                for idx, geom in enumerate(up2you_normals_geoms):
                    if geom is not None:
                        ...
                        if img.shape[2] == 3:
                            img = np.concatenate([img, np.full((*img.shape[:2], 1), 255, dtype=np.uint8)], axis=2)
                        up2you_normals_images_rotated.append(img)
                    else:
                        up2you_normals_images_rotated.append(None)
            ...
            if 'Ours' in rows_to_plot:
                for idx, geom in enumerate(ours_normals_geoms):
                    if geom is not None:
                        ...
                        if img.shape[2] == 3:
                            img = np.concatenate([img, np.full((*img.shape[:2], 1), 255, dtype=np.uint8)], axis=2)
                        ours_normals_images_rotated.append(img)
                    else:
                        ours_normals_images_rotated.append(None)
            
            # Render error images with rotation
            if 'GT' in rows_to_plot:
                for data in gt_error_data:
                    if data is not None:
                        center, camera_pos, max_range = get_view_params_rotated(data['gt_vertices'], rotation_degrees=rotation_degrees)
                        img = render_mesh_pyvista(data['gt_vertices'], data['gt_faces'], colors=None,
                                                 center=center, camera_pos=camera_pos, max_range=max_range,
                                                 opacity=1.0, default_color='lightgray', canvas_size=canvas_size)
                        if img.shape[2] == 3:
                            img = np.concatenate([img, np.full((*img.shape[:2], 1), 255, dtype=np.uint8)], axis=2)
                        gt_error_images_rotated.append(img)
                    else:
                        gt_error_images_rotated.append(None)
            
            if 'UP2You' in rows_to_plot:
                for data in up2you_error_data:
                    if data is not None:
                        center, camera_pos, max_range = get_view_params_rotated(data['pred_vertices'], rotation_degrees=rotation_degrees)
                        img = render_with_error_colors(data, global_min, global_max, colormap, 
                                                      canvas_size=canvas_size, center=center, camera_pos=camera_pos, max_range=max_range)
                        up2you_error_images_rotated.append(img)
                    else:
                        up2you_error_images_rotated.append(None)
            ...
            if 'Ours' in rows_to_plot:
                for data in ours_error_data:
                    if data is not None:
                        ...
                        img = render_with_error_colors(data, global_min, global_max, colormap,
                                                      canvas_size=canvas_size, center=center, camera_pos=camera_pos, max_range=max_range)
                        ours_error_images_rotated.append(img)
                    else:
                        ours_error_images_rotated.append(None)
            
            # Render rotated images (180 degrees from the base rotation)
            if 'GT' in rows_to_plot:
                for data in gt_error_data:
                    if data is not None:
                        center, camera_pos, max_range = get_view_params_rotated(data['gt_vertices'], rotation_degrees=rotation_degrees + 180)
                        img = render_mesh_pyvista(data['gt_vertices'], data['gt_faces'], colors=None,
                                                 center=center, camera_pos=camera_pos, max_range=max_range,
                                                 opacity=1.0, default_color='lightgray', canvas_size=canvas_size)
                        if img.shape[2] == 3:
                            img = np.concatenate([img, np.full((*img.shape[:2], 1), 255, dtype=np.uint8)], axis=2)
                        gt_rotated_images_rotated.append(img)
                    else:
                        gt_rotated_images_rotated.append(None)
            
            if 'UP2You' in rows_to_plot:
                for data in up2you_error_data:
                    if data is not None:
                        center, camera_pos, max_range = get_view_params_rotated(data['pred_vertices'], rotation_degrees=rotation_degrees + 180)
                        img = render_with_error_colors(data, global_min, global_max, colormap, 
                                                      canvas_size=canvas_size, center=center, camera_pos=camera_pos, max_range=max_range)
                        up2you_rotated_images_rotated.append(img)
                    else:
                        up2you_rotated_images_rotated.append(None)
            ...
            if 'Ours' in rows_to_plot:
                for data in ours_error_data:
                    if data is not None:
                        ...
                        img = render_with_error_colors(data, global_min, global_max, colormap,
                                                      canvas_size=canvas_size, center=center, camera_pos=camera_pos, max_range=max_range)
                        ours_rotated_images_rotated.append(img)
                    else:
                        ours_rotated_images_rotated.append(None)
            
            # Create and save the full comparison figure for this rotation
            view_output_path = os.path.join(views_output_dir, f'comparison_view_{view_idx:03d}.png')
            create_interleaved_figure(
                gt_normals_images_rotated, up2you_normals_images_rotated, ours_normals_images_rotated,
                gt_error_images_rotated, up2you_error_images_rotated, ours_error_images_rotated,
                gt_rotated_images_rotated, up2you_rotated_images_rotated, ours_rotated_images_rotated,
                gt_labels, up2you_labels, ours_labels,
                global_min, global_max, rows_to_plot=rows_to_plot, output_path=view_output_path, save_dpi=save_dpi
            )
        
        print('='*60)
        print(f'Finished rendering {num_views} full comparison figures!')
        print('='*60 + '\n')
...
if __name__ == '__main__':
    ...
    output_path = os.path.join('Figures/supplementary', 'vs_comparison_fb.png')

    # Number of subplots per row (stride=1 means all, stride=2 means every 2nd, etc.)
    stride = 1

    # Enable rendering 20 views around the body for each mesh
    render_20_views = True
    views_output_dir = 'Figures/supplementary'  # Directory for saving 20 rotated comparison figures

    main(base_dir=base_dir, stride=stride, rows_to_plot=rows_to_plot, output_path=output_path, 
         save_dpi=save_dpi, canvas_size=canvas_size, render_20_views=render_20_views, 
         views_output_dir=views_output_dir)
